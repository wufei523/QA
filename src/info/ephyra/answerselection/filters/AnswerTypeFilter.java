...
public class AnswerTypeFilter extends Filter {
	...
	public Result[] apply(Result[] results) {
		...
		for (Result result : results) {
			...
			//System.out.println("checking out answerTypes for query " + query.getQueryString() + " : ");
			//System.out.println(Arrays.toString(answerTypes));
			
			if (Search.use_fei_version){
				if (!query.extractWith(ID) || answerTypes.length == 0)
					continue;
			}
			...
			//System.out.println("Doing answer type filtering for result: " + result.getAnswer() + " with score = " + result.getScore());
			...
			for (String answerType : answerTypes) {
				...
				for (int neId : neIds) {
					...
					//for my sample question, ne types are NEtime, NEdate
					System.out.println("NE types are: " + neType);
					
					String[][] nes = NETagger.extractNes(tokens, neId);
					
					for (int i = 0; i < sentences.length; i++) {
						
						// for each sentences, several nes, so are 2d array
		...
		if (Search.print_debugging_info == true){
			System.out.println("new results in this filter are: ");
